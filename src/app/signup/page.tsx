...
export default function SignupPage() {
  ...
  // Common Handler function for all inputs
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    setFormData((prev) => ({
      ...prev,
      [name]: value, // Dynamic key mapping (username, email, password)
    }));
  };
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      // URL: Check backend index.ts to confirm if it's '/api/auth/signup' or '/auth/signup'
      const response = await axios.post('http://localhost:5000/api/auth/signup', formData);
      
      alert("Account created successfully! Redirecting to login...");
      router.push('/login');
    } catch (err: any) {
      // Backend status 400 (All fields required) handle karne ke liye
